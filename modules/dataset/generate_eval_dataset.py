...
def generate_eval_dataset_entry(dataset_name):
    ...
    eval_data = {
        ...
        'legacy_synthetic_j': [],  #
        'legacy_synthetic_jg': [],  #
        'legacy_synthetic_jgo': [],  #
        'legacy_synthetic_jgos': [],  #
        'legacy_real': [],  #
...
def generate_synthetic_data(common_data, eval_data, poses_ref, trans_ref):
    ...
    for noise in ['j', 'jg', 'jgo', 'jgos']:
        ...
        for f in tqdm(range(n_frames), file=sys.stdout, desc=noise):
            aug_range = 0.005
            aug_offset = (torch.rand(1).item() - 0.5) * aug_range
            ...
            temp_marker_indices = to_gpu(temp_marker_indices).to(torch.int32)
            bind_m = to_gpu(common_data.caesar_bind_v[body_idx])[temp_marker_indices].to(torch.float32)
            bind_mn = to_gpu(common_data.caesar_bind_vn[body_idx])[temp_marker_indices].to(torch.float32)
            ...
            m_j3_indices = to_gpu(common_data.v_j3_indices)[temp_marker_indices, :].to(torch.int32)
            m_j3_weights = to_gpu(common_data.v_j3_weights)[temp_marker_indices, :].to(torch.float32)
        ...
        eval_data[f'synthetic_{noise}'].append(to_cpu(points_seq))
    ...
    for noise in ['j', 'jg', 'jgo', 'jgos', 'real']:
        ...
        for m_idx in range(len(eval_data['n_frames'])):
            ...
            if noise == 'real':
                ...
                eval_data[f'legacy_synthetic_{noise}'].append(markers)
