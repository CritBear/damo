...
def main():
    ...
    train_data_count = len(options.train_dataset_paths)
    train_data_indices = torch.arange(train_data_count).chunk(world_size)

    test_data_count = len(options.test_dataset_paths)
    test_data_indices = torch.arange(test_data_count).chunk(world_size)

    torch.multiprocessing.spawn(train,
                                args=(world_size, options, train_data_indices, test_data_indices),
                                nprocs=world_size,
                                join=True)
...
def train(rank, world_size, options, train_data_indices, test_data_indices):
    ...
    train_dataset = DamoDataset(
        ...
        dataset_paths=options.train_dataset_paths[train_data_indices[rank]],
    ...
    test_synthetic_dataset = DamoDataset(
        common_dataset_path=options.common_dataset_path,
        dataset_paths=options.test_dataset_paths[test_data_indices[rank]],
        n_max_markers=options.n_max_markers,
        seq_len=options.seq_len,
        r_ss_ds_ratio=[0, 0.5, 0.5],
        noise_jitter=True,
    ...
    model = Damo(options).to(rank)
    optimizer = torch.optim.Adam(model.parameters(), lr=options.lr)

    if rank == 0:
        log_path = options.log_dir / 'log.txt'
        setup_logging(log_path)
        print(f'INFO | Train | cuda count: {world_size}')
